...
Deno.test("TerragruntDocs - listDocumentsInCategory", async () => {
  ...
  const documents = await terragruntDocs.listDocumentsInCategory(
    "getting-started",
  );
  ...
  console.log(
    `Found ${documents.length} documents in Getting Started category:`,
  );
  ...
  // Check the structure of each document summary
  for (const doc of documents) {
    assertExists(doc.name);
    assertExists(doc.path);
    assertExists(doc.html_url);
    assertExists(doc.download_url); // Should be present even if empty string
...
Deno.test("TerragruntDocs - listDocumentsInCategory - invalid category", async () => {
  ...
  assertEquals(
    error.message.includes('Category "nonexistent-category" not found'),
    true,
    "Error message should indicate the specific category was not found",
  );
});
...
Deno.test("TerragruntDocs - getDocumentFromCategory and listDocumentsInCategory integration", async () => {
  ...
  console.log(
    `Fetching document "${firstDocName}" from Getting Started category`,
  );
  ...
  console.log(
    `Successfully fetched document "${document.name}" with ${document.content.length} characters of content`,
  );
});
...
Deno.test("TerragruntDocs - getAllDocumentsByCategory", async () => {
  ...
  const mergedContent = await terragruntDocs.getAllDocumentsMergedFromCategory(
    categoryName,
  );
  ...
  assertEquals(
    mergedContent.startsWith("# Getting Started Documentation"),
    true,
  ...
  console.log(
    `Successfully fetched merged content from ${categoryName} with ${mergedContent.length} characters`,
  );
  ...
  for (const doc of documents) {
    ...
    assertEquals(
      docHeaderPattern.test(mergedContent),
      true,
  ...
  console.log(
    `Verified all ${documents.length} documents appear in the merged content`,
  );
});
...
Deno.test("TerragruntDocs - getAllDocumentsByCategory - invalid category", async () => {
  ...
  let error: Error | undefined;
  try {
    await terragruntDocs.getAllDocumentsMergedFromCategory("nonexistent-category");
  } catch (e) {
    error = e as Error;
  }
